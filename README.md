...
| 문제 | 원인 | 해결 |
|------|------|------|
| `/ping`, `/api/tour/festival` 등 404 | Render에 최신 `node.js` 미반영 | GitHub에 push 후 Render 대시보드 → **Manual Deploy** |
| 백엔드 첫 응답 30초 지연 | Render 무료 플랜 Sleep | `fetch(/ping)` 웜업 핑으로 선제 깨우기 |
| CORS 에러 (날씨·경로) | `express.json()` 이후 CORS 등록 → OPTIONS preflight 차단 | CORS 미들웨어를 `express.json()` 앞으로 이동 |
| 환경변수 undefined | `.env` 파일 누락 또는 Render 환경변수 미설정 | Render 대시보드 → Environment → `ORS_KEY` / `WEATHER_KEY` / `TOUR_KEY` 등록 |

### Firebase / 인증

| 문제 | 원인 | 해결 |
|------|------|------|
| 로그인 팝업 뜨지 않음 | `firebase.js`의 `firebaseConfig`가 플레이스홀더 상태 | Firebase Console → 프로젝트 설정 → 웹 앱 → SDK 설정값으로 교체 |
| `auth/unauthorized-domain` 오류 | 현재 도메인이 Firebase 허용 도메인 목록에 없음 | Firebase Console → Authentication → Settings → 승인된 도메인에 배포 URL 추가 |
| 저장된 경로 불러오기 실패 (인덱스 오류) | Firestore `uid + createdAt` 복합 인덱스 없음 | 현재 버전은 클라이언트 정렬로 우회 — 인덱스 불필요 |
| `Cross-Origin-Opener-Policy` 콘솔 경고 | Google OAuth 팝업과 브라우저 COOP 정책 충돌 | Google 서버 정책이므로 코드로 제거 불가, 기능 동작에 무관 |

### 데이터 / API

| 문제 | 원인 | 해결 |
|------|------|------|
| 종료된 행사(2025 등)가 표시됨 | TourAPI location API는 행사 날짜 필드를 반환하지 않음 | 행사(`contenttypeid=15`)는 location·keyword API에서 완전 제거, `/api/tour/festival`(서버 재배포 필요)로만 조회 |
| 이미지 Mixed Content | TourAPI가 `http://` 이미지 URL 반환 | 출력 전 `replace(/^http:\/\//, "https://")` 처리 |
| 관광지 0건 | 설정 반경 내 데이터 없음 | 결과 0건 시 반경 자동 10km 확장 재시도 |
| 운영시간·휴일 표시 안 됨 | location API는 운영정보 미포함 | 모달 오픈 시 `/api/tour/intro`(`detailIntro2`) 별도 호출 (서버 재배포 필요) |
| 거리가 GPS 허용 전에도 표시됨 | `dist` 필드가 기본 좌표 기준으로 계산됨 | `gpsGranted` 플래그로 GPS 허용 후에만 거리 렌더링 |

### 로컬 개발

| 문제 | 원인 | 해결 |
|------|------|------|
| `festival API 오류: Unexpected token '<'` | 로컬에서 프록시 서버 미실행 | `node node.js` 실행 후 `PROXY_BASE`를 `http://localhost:4000`으로 변경 |
| `firebase.js` 모듈 오류 | 로컬 파일 직접 열기 (`file://`) | Live Server 등 HTTP 서버로 실행 필요 |
