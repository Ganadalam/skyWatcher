...
async function fetchRouteMinutes(lng1,lat1,lng2,lat2,profile) {
  try {
    const res = await fetch(`${PROXY_BASE}/route`, {
      ...
      body:JSON.stringify({ coordinates:[[lng1,lat1],[lng2,lat2]], profile }),
...
function apiLocationBased(lat,lng,radius,contentTypeId="") {
  ...
  return fetch(`${PROXY_BASE}/api/tour/location?${qs}`)
    .then(r=>r.json()).then(d => parseTourItems(d, true))
...
async function checkServerAlive() {
  ...
  try {
    const r = await fetch(`${PROXY_BASE}/ping`, { signal: AbortSignal.timeout(3000) });
    _serverAlive = r.ok;
...
async function apiFestival(lat, lng) {
  ...
  try {
    const r = await fetch(`${PROXY_BASE}/api/tour/festival${qs}`);
    if (!r.ok) return [];
...
async function fetchPlaceDetail(contentId) {
  ...
  try {
    const r = await fetch(`${PROXY_BASE}/api/tour/detail?contentId=${contentId}`);
    if (!r.ok) return null;
    const data = await r.json();
...
async function fetchPlaceIntro(contentId, contentTypeId) {
  if (!contentId) return null;
  try {
    const r = await fetch(`${PROXY_BASE}/api/tour/intro?contentId=${contentId}&contentTypeId=${contentTypeId||12}`);
    if (!r.ok) return null;
    const data = await r.json();
    const item = data?.response?.body?.items?.item;
    if (!item) return null;
...
async function fetchWeatherByCoords(lat,lng) {
  try {
    const r = await fetch(`${PROXY_BASE}/api/weather/coords?lat=${lat}&lon=${lng}`);
    if (!r.ok) return null;
    return await r.json();
  } catch(e) { return null; }
}
async function fetchWeatherByCity(city) {
  try {
    const r = await fetch(`${PROXY_BASE}/api/weather/city?q=${encodeURIComponent(city)}`);
    if (!r.ok) return null;
    return await r.json();
  } catch(e) { return null; }
}
async function fetchForecastByCoords(lat,lng) {
  try {
    const r = await fetch(`${PROXY_BASE}/api/weather/forecast?lat=${lat}&lon=${lng}`);
    if (!r.ok) return null;
    return await r.json();
  } catch(e) { return null; }
}
  ...
  if (p.contentid) {
    ...
    fetchPlaceIntro(p.contentid, p.contenttypeid).then(info => {
      ...
      const set = (id, val) => {
        if (val && val.trim()) {
          $(`#${id}`).show();
          $(`#${id}-val`).text(val.trim());
          hasAny = true;
        }
      };
      set("modal-usetime",  info.usetime);
      set("modal-restdate", info.restdate);
      set("modal-parking",  info.parking);
...
$(document).ready(function() {
  ...
  // 초기 스켈레톤 즉시 표시 후 콘텐츠 로딩
  showWeatherUnavailable();
  showSkeletonGrid(6);
  // main-content는 항상 노출 상태 유지 (로딩 시 오버레이로 처리)

  // 서버 웜업 핑 (Render 콜드스타트 방지) - 병렬로
  fetch(`${PROXY_BASE}/ping`).then(r => r.ok ? r.json() : null).catch(()=>{});
  ...
  })().catch(()=>showError("관광지 데이터를 불러오지 못했습니다. 네트워크를 확인해주세요."));
});
